            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-background/50 p-3 border border-border/50 text-center">
                ...
                <p className="text-lg font-bold text-red-400">
                  {client.encours.toLocaleString("fr-FR")}€
                </p>
              </div>
              <div className="rounded-lg bg-background/50 p-3 border border-border/50 text-center">
                <p className="text-xs text-muted-foreground">DSO</p>
                <p className={`text-lg font-bold ${client.dso > 30 ? "text-amber-400" : "text-emerald-400"}`}>
                  {client.dso}j
                </p>
              </div>
              <div className="rounded-lg bg-background/50 p-3 border border-border/50 text-center">
                ...
                <p className="text-lg font-bold text-foreground">
                  {(client.totalFacture / 1000).toFixed(0)}k€
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {invoices.map((invoice) => (
                <div 
                  key={invoice.id}
                  className="flex items-center justify-between rounded-lg bg-background/50 p-2.5 border border-border/50 hover:bg-background/70 transition-colors cursor-pointer"
                >
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 lg:p-8 pb-24">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Tableau de bord cash-flow freelance BE/FR
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        {/* KPI Cards - With icons and vs objectif */}
            ...
            <CardContent>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold text-primary">
                  {kpiData.caMtd.toLocaleString("fr-FR")}€
                </span>
                <span className="text-xs text-muted-foreground">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground whitespace-nowrap">
                  vs {kpiData.caObjectif.toLocaleString("fr-FR")}€
                </span>
              </div>
            </CardContent>
          </Card>

          {/* Encours Total - RED + LIVE */}
          <Card className="bg-card/50 backdrop-blur-sm border-red-500/30">
            <CardHeader className="pb-2">
              <CardTitle className="text-xs font-medium text-muted-foreground flex items-center gap-2">
                <Activity className="h-3.5 w-3.5 text-red-400" />
                Encours Total
                <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 bg-red-500/10 text-red-400 border-red-500/30 animate-pulse">
                  LIVE
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold text-red-400">
                  {kpiData.encoursTotal.toLocaleString("fr-FR")}€
                </span>
              </div>
            ...
            <CardContent>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold text-foreground">
                  {kpiData.bceVerifies}/{kpiData.bceTotal}
                </span>
              </div>
              <div className="mt-2 flex items-center gap-1 text-xs text-emerald-400">
                <CheckCircle2 className="h-3 w-3" />
                <span>Conformité active</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {/* Alertes Card - Functional buttons */}
            ...
            <CardContent className="space-y-3">
              {alertes.map((alerte) => (
                <div
                  key={alerte.id}
                  className="flex items-center justify-between rounded-lg bg-background/50 p-2.5 border border-border/50 hover:bg-background/70 transition-colors cursor-pointer"
                  onClick={() => handleAlertClick(alerte.clientId)}
                >
                  <div className="space-y-0.5">
                    <p className="text-sm font-medium">{alerte.client}</p>
                    <p className="text-xs text-muted-foreground">
                      {alerte.invoice} · <span className="text-red-400">+{alerte.retard}j</span>
                    </p>
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      // TODO: Trigger relance via Supabase
                    }}
                  >
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={caMtdData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    ...
                    content={
                      <CustomTooltip
                        formatter={(value, name) =>
                          `${name === "actuel" ? "Actuel" : "Objectif"}: ${value.toLocaleString("fr-FR")}€`
                        }
                      />
                    }
                  />
                  ...
                  <Line
                    ...
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 5, fill: "oklch(0.75 0.18 85)" }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                Évolution Encours
                <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 bg-red-500/10 text-red-400 border-red-500/30">
                  Live
                </Badge>
              </CardTitle>
            </CardHeader>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={encoursData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    content={
                      <CustomTooltip
                        formatter={(value) => `Encours: ${value.toLocaleString("fr-FR")}€`}
                      />
                    }
                  />
                  <Area
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 h-9 w-full sm:w-[200px] bg-background/50 border-border/50"
                  />
                </div>
                <DropdownMenu>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border/50 hover:bg-transparent">
                  ...
                  <TableHead 
                    className="text-muted-foreground text-center cursor-pointer hover:text-foreground transition-colors"
              ...
              <TableBody>
                {filteredAndSortedClients.map((client) => (
                  <TableRow
                    key={client.id}
                    className="border-border/50 cursor-pointer hover:bg-primary/5 hover:backdrop-blur-sm transition-all group"
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-amber-400">
                      Top 3 Clients à Risque (DSO {">"}30j)
                    </span>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    {riskClients.map((client) => (
                      <div
                        key={client.id}
                        className="flex items-center gap-2 rounded-lg bg-background/50 px-3 py-1.5 border border-border/50 cursor-pointer hover:bg-background/70 hover:border-amber-500/50 transition-all"
                        onClick={() => handleClientClick(client)}
                      >
